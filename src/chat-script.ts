...
export function getChatScript(brandIconUri: string): string {
  return `
    ...
    function formatCommandRiskLabel(risk) {
      if (risk === "long-running") return "long-running";
      if (risk === "writes") return "write";
      return risk || "caution";
    }

    function getCommandRiskNote(risk) {
      if (risk === "safe") return "PocketAI wants to run a read-only or verification command.";
      if (risk === "network") return "PocketAI wants to run a command that may use the network or install dependencies.";
      if (risk === "long-running") return "PocketAI wants to run a command that may keep running until stopped.";
      if (risk === "writes") return "PocketAI wants to run a command that may write files in your workspace.";
      if (risk === "destructive") return "PocketAI wants to run a command classified as destructive.";
    ...
    function renderHarnessPane(payload) {
      ...
      if (changeSets.length) {
        ...
        for (const changeSet of changeSets) {
          ...
          viewBtn.className = "tool-btn";
          viewBtn.textContent = "View Set";
          viewBtn.disabled = !!payload.busy;
          viewBtn.onclick = () =>
